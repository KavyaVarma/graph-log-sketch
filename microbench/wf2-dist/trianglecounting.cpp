...
int main(int argc, char** argv) {
	...
	for (uint32_t i=0; i<num_hosts; i++) {
		file << "host " << host_id << " remote read to host " << i << ": " << remote_read_to_host[i].read_local() << std::endl;
		file << "host " << host_id << " remote write to host " << i << ": " << remote_write_to_host[i].read_local() << std::endl;
	}
    ...
    galois::do_all(
        ...
        [&](const PGNode& src) {
			...
			for (auto global_dst : srcData.dests) {
                ...
                *thread_num_tri_ptr += intersect_merge<NodeData, void>(hg_ref, src, global_dst, local_read_stream, local_read_random, remote_read_stream, remote_write_stream);
            }
        },
